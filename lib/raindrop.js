...
const getTimestamp =
  (date) => parseInt(date / constants.second, constants.radix10)

const getMaterials = (arg, date) => {
  const newTimestamp = getTimestamp(date)

  if (newTimestamp > shared.lastTimestamp || util.isNil(shared.lastTimestamp)) {
    shared.lastTimestamp = newTimestamp
...
const raindrop = (arg) => {
  ...
  materials.timestamp =
    new Date(materials.timestamp * constants.second).toISOString()
