...
// 255 - 8bit - Tiny Int
const uTinyIntMax = 0xff;

// 16777215 - 24bit - Medium Int
const uMedIntMax = 0xffffff;

// 2147483647 - 32-bit - Int
...
const isUTinyInt = (num) => num >= 0 && num <= uTinyIntMax;

const isUMedInt = (num) => num >= 0 && num <= uMedIntMax;
...
const generate = (arg) => {
  const timestamp = parseInt(Date.now() / second, radix10) % uIntMax;
  ...
  const rand = parseInt(Math.random() * uMedIntMax, radix10) % uIntMax;
