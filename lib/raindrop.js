'use strict';
var Config, Pack, Raindrop, Util, buffer, cycle, generate, hex, index, lastTimestamp, next, version;
...
Raindrop = function(arg) {
  ...
  if (arg && ((arg instanceof Raindrop) || arg.raindropType === version)) {
    return arg;
  }
  buf = void 0;
...
cycle = function(timestamp) {
  ...
  if (timestamp > lastTimestamp) {
    lastTimestamp = timestamp;
    return index = parseInt(Math.random() * 0xFFFFFF, 10);
  }
};

generate = function(arg) {
  var eid, pid, ref, ref1, ref2, sid, time, timestamp;
  timestamp = parseInt(Date.now() / 1000, 10);
  cycle(timestamp);
  time = timestamp % 0xFFFFFFFF;
  pid = (ref = arg != null ? arg.processId : void 0) != null ? ref : Config.defaultPid;
  sid = (ref1 = arg != null ? arg.serviceId : void 0) != null ? ref1 : Config.defaultSid;
  eid = (ref2 = arg != null ? arg.entityTypeId : void 0) != null ? ref2 : Config.defaultEid;
  if (!(Util.isUmedInt(pid))) {
    throw new Error('Process Id must be between 0 and 16,777,214');
  }
  ...
  return hex(8, time) + hex(6, pid) + hex(2, sid) + hex(2, eid) + hex(6, next());
};
